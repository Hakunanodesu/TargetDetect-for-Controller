...
class XboxWirelessToX360Mapper:
    """
    XboxWirelessToX360Mapper 类：
    - 使用 pywinusb.hid 读取 Xbox 无线控制器的原始 HID 输入
    - 使用 vgamepad 创建虚拟 Xbox 360 手柄，并将 Xbox 无线控制器输入映射到虚拟手柄
    - 提供 start() 和 stop() 方法，方便在外部控制映射循环的启动和停止
    """

    def __init__(self, product_id: int, path: str):
        """
        初始化 XboxWirelessToX360Mapper。

        :param vendor_id: Xbox 无线控制器供应商 ID，默认使用 Microsoft（0x045E）
        :param product_id: Xbox 无线控制器产品 ID，默认使用 Xbox 360 手柄（0x028E）
    ...
    def _map_to_x360(self):
        ...
        if (touchpad_ps & 0x01) != 0:
            self.virtual_gamepad.press_button(button=vg.XUSB_BUTTON.XUSB_GAMEPAD_GUIDE)
        else:
            self.virtual_gamepad.release_button(button=vg.XUSB_BUTTON.XUSB_GAMEPAD_GUIDE)
        # 触控板映射到Back（额外）
...
class DualSenseToX360Mapper:
    ...
    def _map_to_x360(self):
        ...
        touchpad_ps = self.dual_sense_state["touchpad_ps"]
        # Back；触控板映射到Back（额外）
        if (sticks & 0x10) != 0 or (touchpad_ps & 0x02) != 0:
            self.virtual_gamepad.press_button(button=vg.XUSB_BUTTON.XUSB_GAMEPAD_BACK)
        else:
            self.virtual_gamepad.release_button(button=vg.XUSB_BUTTON.XUSB_GAMEPAD_BACK)
        # Start
